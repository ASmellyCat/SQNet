...
lambda1 = 1.0  # Weight for Coverage Loss
lambda2 = 1.0  # Weight for Consistency Loss
epsilon = 0.1  # Threshold for surface points
...
def main():
    ...
    for obj_name in object_names:
        ...
        for epoch in range(num_epochs):
            ...
            # ====== Coverage Loss (L1) ======
            # Mask for surface points (|S(O)| < epsilon)
            mask_surface = torch.abs(values) < epsilon
            if mask_surface.sum() > 0:
                coverage_loss = torch.mean(cuboid_sdf[mask_surface] ** 2)
            else:
                coverage_loss = torch.tensor(0.0, device=device)

            # ====== Consistency Loss (L2) ======
            # Mask for interior points (S(O) < 0)
            mask_inside = values < 0
            if mask_inside.sum() > 0:
                # For interior points, encourage cuboid_sdf to be <= 0
                consistency_loss = torch.mean(torch.clamp(cuboid_sdf[mask_inside], min=0.0) ** 2)
            else:
                consistency_loss = torch.tensor(0.0, device=device)

            # ====== Total Loss ======
            loss = mseloss + lambda1 * coverage_loss + lambda2 * consistency_loss
            ...
            if (epoch + 1) % 50 == 0 or epoch == 0:
                print(f"Epoch {epoch + 1}/{num_epochs}, "
                      f"MSE Loss: {mseloss.item():.6f}, "
                      f"Coverage Loss (L1): {coverage_loss.item():.6f}, "
                      f"Consistency Loss (L2): {consistency_loss.item():.6f}, "
        ...
        visualise_cuboids(cuboid_params, reference_model=pcd_model, 
                         save_path=os.path.join(output_dir, f"{obj_name}_cuboids.obj"))
